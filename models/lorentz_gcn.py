...
from layers import LorentzLayer, ALL_STACK_LAYERS, LightLayer, HyperbolicLayer
...
class LorentzGCN(Base):
    def __init__(self, num_users, num_items, graph, args):
        ...
        # layers = torch.nn.ModuleList([LorentzLayer(self.manifold, self.c) for _ in range(args.num_layers)])
        # self.convs = ALL_STACK_LAYERS[args.stack](layers, args.num_layers, self.graph)
